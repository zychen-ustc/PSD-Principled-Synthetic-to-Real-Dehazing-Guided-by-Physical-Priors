...
class RandomRotationFromSequence(object):
    ...
    def __call__(self, data, num_data=3):
        """
        ...
        return F.rotate(hr, angle, self.resample, self.expand, self.center), \
                F.rotate(lr, angle, self.resample, self.expand, self.center), \
                 F.rotate(ex, angle, self.resample, self.expand, self.center)
...
class RandomHorizontalFlip(object):
    ...
    def __call__(self, data, num_data=3):
        ...
        if random.random() < 0.5:
            return F.hflip(hr), F.hflip(lr), F.hflip(ex)
        return hr, lr, ex
...
class RandomVerticalFlip(object):
    ...
    def __call__(self, data, num_data=3):
        ...
        if num_data == 2:
            ...
            hr, lr, ex = data
        if random.random() < 0.5:
            return F.vflip(hr), F.vflip(lr), F.vflip(ex)
        return hr, lr, ex
...
class RandomCrop(object):
    ...
    def __call__(self, data, num_data=3):
        ...
        if self.padding > 0:
            ...
            ex = F.pad(ex, self.padding)

        i, j, h, w = self.get_params(data, self.size)
        return F.crop(hr, i, j, h, w), F.crop(lr, i, j, h, w), F.crop(ex, i, j, h, w)
...
class ToTensor(object):
    ...
    def __call__(self, data, num_data=3):
        ...
        if num_data == 2:
            ...
            hr, lr, ex = data
        return F.to_tensor(hr), F.to_tensor(lr), F.to_tensor(ex)
